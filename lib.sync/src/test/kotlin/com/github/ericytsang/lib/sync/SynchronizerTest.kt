...
{
    val events = (0..3).map {Event(it)}

    val master = spy(MockMaster())
    val slaves = (0..1).map {spy(MockSlave())}

    val unlatchToLetThreadsRun = CountDownLatch(1)
    val fixture = Synchronizer(master,ThreadFactory()
    {
        thread(start = false)
        {
    ...
    fun events_received_after_slaves_are_connected_are_sent_to_slaves()
    {
        ...
        for (slave in slaves)
        {
            `when`(slave.getPendingRequests()).thenReturn(emptyList())
        }

        // connect slaves to the fixture
        val fixture = Synchronizer(master,Executors.defaultThreadFactory())
